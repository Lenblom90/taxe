...
import { createdatToString, estimateToString, priorityToString } from "../lib/TaskHelperMethods";
...
if(editable){
      ...
      <form method="post" onSubmit={handleSubmit}>
        <div className="form-header">
          <input
            ...
            onChange={(e) => setTask({...currentTask, completed_at: e.target.checked ? new Date().toISOString() : undefined})}
            type="checkbox"
          ></input>
          <input
            id="taskName"
            value={currentTask.name}
            onChange={(e) => setTask({...currentTask, name: e.target.value})}
            type="text"
          ></input>
          <div>created on {new Date(Date.parse(task.created_at)).toDateString()}</div>
        </div>
        <div className="form-body">
          <textarea
            value={currentTask.note}
            onChange={(e) => setTask({...currentTask, note: e.target.value})}
            name="taskDescription"
            placeholder="description...."
          ></textarea>
          <TaskSchedule handleSchedule={(scheduled_on : string) => setTask({...currentTask, scheduled_on: scheduled_on})} show={scheduler} />
        ...
        <div className="form-footer">
          <div className="form-options">
            <select
              ...
              onChange={(e) => setTask({...currentTask, status: e.target.value})}
              name="taskStatus"
            >
            ...
            <select
              ...
              onChange={(e) => setTask({...currentTask, estimate: Number.parseInt(e.target.value)})}
              name="taskEstimate"
            >
            ...
            <select
              ...
              onChange={(e) => setTask({...currentTask, priority: Number.parseInt(e.target.value)})}
              name="taskPriority"
            >
          ...
          <div className="form-icons">
            ...
            <img onClick={() => setScheduler(!scheduler)} src={book} alt="book-icon"/>
            <img onClick={() => setTimer(!timer)} src={hourglass} alt="hourglass-icon"/>
            <img onClick={() => setRepeat(!repeat)} src={sundial} alt="sundial-icon"/>
          </div>
        </div>
  ...
  return( 
    <li draggable={true} onDragStart={() => setDrag(task)} onClick={() => setEditable(true)} className="tasklist-item">
      ...
      <div className="age">
        {createdatToString(task.created_at)}
      </div>
